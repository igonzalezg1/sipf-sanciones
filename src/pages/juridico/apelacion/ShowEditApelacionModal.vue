      ...
      <q-card-section>
        <div class="q-mb-md">
          ...
          <q-form ref="formulario">
            ...
            <q-uploader
              v-if="!props.isReadonlyApelacion"
              :url="uploadUrl"
              method="POST"
...
// Validators
import EditValidator from 'src/app/validators/apelacion/edit.validator';
...
const formData = ref<ApelacionCreate>({
  cuando_aplica: sancion.value?.apelacion?.cuando_aplica ?? '',
  fecha_solicitud: sancion.value?.apelacion?.fecha_solicitud ?? '',
  numero_sesion: sancion.value?.apelacion?.no_sesion_comite ?? '',
  organo_jurisdiccional: sancion.value?.apelacion?.organo_jurisdiccional ?? '',
  observaciones: sancion.value?.apelacion?.observaciones ?? '',
  apelacion_file: null,
});
