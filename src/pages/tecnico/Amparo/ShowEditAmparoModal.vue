...
const formData = ref<AmparoCreate>({
  cuando_aplica: sancion.value?.amparo?.cuando_aplica ?? '',
  fecha_solicitud: sancion.value?.amparo?.fecha_solicitud ?? '',
  numero_sesion: sancion.value?.amparo?.no_sesion_comite ?? '',
  organo_jurisdiccional: sancion.value?.amparo?.organo_jurisdiccional ?? '',
  observaciones: sancion.value?.amparo?.observaciones ?? '',
  amparo_file: null,
});
  ...
  await formulario.value.validate().then(async (exito: boolean) => {
    ...
    if (filename.path) {
      try {
        ...
        const response = await amparoService.agregarSancion(
          incidente_id,
          sancion_id,
          formData.value,
        );
