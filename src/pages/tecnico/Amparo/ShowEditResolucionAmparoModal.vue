  ...
  >
    <q-card style="width: 800px; max-width: 95vw">
      <q-card-section class="text-center text-white bg-primary tw-shadow-lg">
        <div class="text-h6">Editar resolucion de amparo.</div>
      </q-card-section>
        ...
        <div class="q-mb-md">
          <q-form ref="formulario">
            ...
            <q-uploader
              ...
              @added="onFileAdded"
              label="Agregar resolución de amparo (máx. 10MB)"
              :auto-upload="false"
              field-name="file"
      ...
      <q-card-actions align="center">
        <q-btn label="Guardar" v-if="!props.isReadOnly" color="positive" @click="saveInfo" />
        <q-btn label="limpiar" v-if="!props.isReadOnly" color="info" @click="clearform" />
...
// Modelos
import type { AmparoResolucionCreate } from 'entities/amparo/amparo.model';
...
const formData = ref<AmparoResolucionCreate>({
  fecha_resolucion: parseToInputDate(sancion.value?.amparo?.fecha_resolucion ?? ''),
  fecha_inicio_sancion: parseToInputDate(sancion.value?.amparo?.fecha_inicio_sancion ?? ''),
  fecha_fin_sancion: parseToInputDate(sancion.value?.amparo?.fecha_fin_sancion ?? ''),
  observaciones_resolucion: sancion.value?.amparo?.observaciones_resolucion ?? '',
  resolucion_juez: sancion.value?.amparo?.resolucion_juez ?? '',
  amparo_resolucion_file: '',
  fecha_suspencion: parseToInputDate(sancion.value?.amparo?.fecha_suspencion ?? ''),
});
...
function goToFile() {
  const actualFile = `${storageURL.value}/${sancion.value?.amparo?.file_name_resoluccion ?? ''}`;
